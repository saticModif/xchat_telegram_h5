...
import { parseInitialLocationHash, parseChannelUsernameFromPath, isChannelUrl } from '../util/routing';
...
const App: FC<StateProps> = ({
  ...
  // 检查是否为频道URL
  if (isChannelUrl()) {
    channelUsername = parseChannelUsernameFromPath();
    // 继续使用主界面，频道打开将在 useEffect 中处理
  ...
  // 处理频道URL访问 - 延迟调用确保应用完全初始化
  useEffect(() => {
    if (isChannelUrl() && authState === 'authorizationStateReady') {
      const username = parseChannelUsernameFromPath();
      if (username) {
        // 延迟执行，确保应用完全初始化
        setTimeout(() => {
          try {
            const { openChannelByUrl } = getActions();
            openChannelByUrl({ username });
          } catch (error) {
            console.error('Failed to open channel by URL:', error);
          }
        }, 100); // 100ms 延迟，确保应用状态完全准备好
      }
    }
  }, [authState]);
  ...
  function renderContent() {
    ...
    switch (activeKey) {
      ...
      case AppScreens.auth:
        {
          if(window.setShowAppTabs && window.logout){

            window?.setShowAppTabs(false)
            window?.logout()
          }
          return <Auth />;
        }
