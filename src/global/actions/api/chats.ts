...
addActionHandler('openChannelByUrl', async (global, actions, payload): Promise<void> => {
  const { username, tabId = getCurrentTabId() } = payload;

  try {
    console.log('Opening channel by URL:', username);
    // 直接调用现有的 openChatByUsername 功能
    await openChatByUsername(global, actions, { username }, tabId);
  } catch (error) {
