  ...
  switch (update['@type']) {
    ...
    case 'updateCurrentUser':
      ...
      const obj = {
        ...
        phoneNumber:update?.currentUser?.phoneNumber,
        nick : `${update?.currentUser?.firstName} ${update?.currentUser?.lastName}`,
        
      }
      if(update?.currentUser.usernames && update?.currentUser.usernames.length>0){
      
        obj.username = update.currentUser.usernames[0].username
      }
      
      // 注释掉有问题的loadUserProfilePhotos调用
      // window.setTimeout(async()=>{
      //   actions.loadUserProfilePhotos({
      //     peerId:update?.currentUser.id,
...
function onUpdateCurrentUser<T extends GlobalState>(global: T, update: ApiUpdateCurrentUser) {
  ...
  // 缓存telegramId
  localStorage.setItem('telegram_user_id', currentUser.id);
}

// 使用从sessions.ts导入的updateSessionUserId函数
