...
// 修改：解析频道用户名URL - 使用hash路由
export function parseChannelUsernameFromPath(): string | undefined {
  // 首先检查hash路由
  const hash = window.location.hash;
  if (hash && hash.startsWith('#/c/')) {
    const username = hash.substring(4); // 移除 '#/c/'
...
// 修改：创建频道URL - 使用hash路由
export function createChannelUrl(username: string): string {
...
// 修改：检查是否为频道URL
export function isChannelUrl(): boolean {
  return !!parseChannelUsernameFromPath();
}
